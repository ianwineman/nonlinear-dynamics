...
function generate_image(size,theme)
	tspan = (0.0,100.0)
	p = [1.0, 0.2, 1.0, 0.3]
	...
	for i in 1:size[1]
		...
		@threads for j in 1:size[2]
			t = trajectory(magnetic_pendulum!, [xs[i],ys[j],0.0,0.0], tspan, p)
			cm = closest_magnet(t[end][1:2]...)
			img[i, j] = colors[cm]
		end
	end
	return img
...
# 2556 x 2556
img = generate_image((400,400), :lapaz) 
save("plots/magnetic_pendulum.png", rotl90(img))
