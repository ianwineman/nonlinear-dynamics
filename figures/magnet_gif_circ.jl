...
#=
# Generate gif of moving magnets (new) path
using Plots; ENV["GKSwstype"] = "100"

θ = vcat(range(-π, 0; length=50), range(0, π; length=50))
anim = @animate for i in 1:100
	plot(cos.(θ), sin.(θ), aspect_ratio=:equal, lc=:black, lw=1.5, label=false, xlim=[-5, 5], ylim=[-5, 5], xtick=[-5, 0, 5], ytick=[-5, 0, 5])
	scatter!([-0.5, -0.5], [sqrt(3)/2, -sqrt(3)/2], mc=:blue, ms=6, label=false)
	plot!(sqrt(0.5) .* cos.(θ) .+ (sqrt(0.5) + 1) , sqrt(0.5) .* sin.(θ), lc=:black, lw=1.5, ls=:dash, label=false)
	scatter!([sqrt(0.5) * cos(θ[i]) + (sqrt(0.5) + 1)], [sqrt(0.5) * sin(θ[i])], mc=:red, ms=6, label=false)
end
gif(anim, "plots/mp/mp_circ_2.gif", fps=30)
=#
...
function generate_gif(size, theme)
	θs = vcat(range(-π, 0; length=50), range(0, π; length=50))
	imgs = []
	@showprogress for θ in θs
		m1 = sqrt(0.5) * cos(θ) + (sqrt(0.5) + 1), sqrt(0.5) * sin(θ)
		img = generate_image(size, theme; m1=m1)
		push!(imgs, img)
	end
	save("plots/mp/mp_circ_basins_$(size[1])x$(size[2])_$(string(theme))_tan.gif", cat(imgs..., dims=3); fps=10)
	println("plots/mp/mp_circ_basins_$(size[1])x$(size[2])_$(string(theme))_tan.gif")
end

generate_gif((1_000, 1_000), :hawaii)
=#

#=
# Generate gif of moving magnets (new-new) path
using Plots; ENV["GKSwstype"] = "100"
...
anim = @animate for i in 1:100
	plot(cos.(θ), sin.(θ), aspect_ratio=:equal, lc=:black, lw=1.5, label=false, xlim=[-5, 5], ylim=[-5, 5], xtick=[-5, 0, 5], ytick=[-5, 0, 5])
	scatter!([-0.5, -0.5], [sqrt(3)/2, -sqrt(3)/2], mc=:blue, ms=6, label=false)
	plot!(sqrt(1/4) .* cos.(θ) .+ (sqrt(1/4) + 1) , sqrt(1/4) .* sin.(θ), lc=:black, lw=1.5, ls=:dash, label=false)
	scatter!([sqrt(1/4) * cos(θ[i]) + (sqrt(1/4) + 1)], [sqrt(1/4) * sin(θ[i])], mc=:red, ms=6, label=false)
end
gif(anim, "plots/mp/mp_circ_3.gif", fps=30)
=#
...
function generate_gif(size, theme)
	...
	imgs = []
	@showprogress for θ in θs
		m1 = sqrt(0.25) * cos(θ) + (sqrt(0.25) + 1), sqrt(0.25) * sin(θ)
		img = generate_image(size, theme; m1=m1)
		push!(imgs, img)
	end
	save("plots/mp/mp_circ_basins_$(size[1])x$(size[2])_$(string(theme))_tan_small.gif", cat(imgs..., dims=3); fps=10)
	println(pwd() * "/plots/mp/mp_circ_basins_$(size[1])x$(size[2])_$(string(theme))_tan_small.gif")
end

generate_gif((1_000, 1_000), :PuOr_3)
